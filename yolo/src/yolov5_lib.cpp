...
void *yolov5_trt_create(const char *engine_name) {
  ...
  std::cout << "[trace] yolov5_trt_create" << std::endl;
  if (file.good()) {
  ...
  std::cout << "[trace] yolov5_trt_create: buffer" << std::endl;
  ...
  std::cout << "[trace] yolov5_trt_create: stream" << std::endl;
  CUDA_CHECK(cudaStreamCreate(&trt_ctx->cuda_stream));
  // cudaStreamCreate(&trt_ctx->cuda_stream);
  std::cout << "[trace] yolov5_trt_create: done" << std::endl;
  return (void *)trt_ctx;
}
...
int yolov5_trt_detect(void *h, cv::Mat &img, float threshold, std::vector<DetectBox> &det) {
  ...
  //  std::cout << "[dbg] delay_proress:" << delay_preprocess << "ms, " << "delay_infer:" << delay_infer << "ms" << std::endl;
