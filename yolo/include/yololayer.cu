...
size_t YoloLayerPlugin::getSerializationSize() const TRT_NOEXCEPT {
  const size_t class_size = sizeof(mClassCount);
  const size_t thread_size = sizeof(mThreadCount);
  const size_t kernel_size = sizeof(mKernelCount);
  const size_t yolo_size = sizeof(Yolo::YoloKernel) * mYoloKernel.size();
  const size_t net_dims = sizeof(mYoloV5NetWidth) + sizeof(mYoloV5NetHeight);
  const size_t output_size = sizeof(mMaxOutObject);
  return class_size + thread_size + kernel_size + yolo_size + net_dims + output_size;
}
...
Dims YoloLayerPlugin::getOutputDimensions(int index, const Dims *inputs, int nbInputDims) TRT_NOEXCEPT {
  ...
  const int totalsize = mMaxOutObject * sizeof(Detection) / sizeof(float);
...
void YoloLayerPlugin::forwardGpu(const float *const *inputs, float *output, cudaStream_t stream, int batchSize) {
  ...
  for (unsigned int i = 0; i < mYoloKernel.size(); ++i) {
    ...
    const int dim_size = (numElem + mThreadCount - 1) / mThreadCount;
    const int block_size = mThreadCount;
    CalDetection<<<dim_size, block_size, 0, stream>>>(inputs[i], output, numElem, mYoloV5NetWidth, mYoloV5NetHeight, mMaxOutObject, yolo.width, yolo.height, (float *)mAnchor[i], mClassCount, outputElem);
  }
}
