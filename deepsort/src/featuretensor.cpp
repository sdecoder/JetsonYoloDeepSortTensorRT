...
void FeatureTensor::loadEngine(std::string enginePath) {
  // Deserialize model
  std::cout << "[trace] In func:FeatureTensor::loadEngine " << enginePath << std::endl;
...
void FeatureTensor::loadOnnx(std::string onnxPath) {
  ...
  /* persist to disk for the generated engine file */
  const auto trtModelStream = engine->serialize();
  boost::filesystem::path path(onnxPath);
  const char *baseFileName = path.filename().c_str();
  const char *replaced_string = path.filename().replace_extension(boost::filesystem::path(".engine")).c_str();
  const std::string finalFileName = std::string(replaced_string);
  std::cout << "[dbg] the final file name is: " << finalFileName << std::endl;
  std::ofstream output_stream(finalFileName);
  output_stream.write((const char *)trtModelStream->data(), trtModelStream->size());
  output_stream.close();
